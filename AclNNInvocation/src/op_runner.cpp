...
bool OpRunner::RunOp()
{
    ...
    INFO_LOG("workspaceSize: %lu\n", workspaceSize);
    aclOpExecutor *handle = nullptr;
    //添加计算workspace大小并申请内存代码
    auto ret = aclnnSigmoidCustomGetWorkspaceSize(inputTensor_[0], outputTensor_[0],
    ...
    //INFO_LOG("workspaceSize: %lu\n", workspaceSize);
    //添加执行算子代码
    ret = aclnnSigmoidCustom(workspace , workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnSigmoidCustom success");
